...
def app():
    ...
    st.write(
        "- Developed a system for retrieving similar images based on their content."
    )
    ...
    st.write(
        "- Developed a model for classifying images using convolutional neural networks (CNNs)."
    )
    ...
    st.write(
        "- Developed a model for generating lyrics using character-level recurrent neural networks (RNNs)."
    )
    ...
    st.write(
        "- Developed a system for object recognition using Raspberry Pi and camera module."
    )
    ...
    st.markdown(
        "**Tech Stack**: Python, ROS, Gazebo, OpenCV, TensorFlow/Keras, NumPy, Pandas, SciPy"
    )

    st.subheader("Project 3: Data Pipeline for Image Segmentation")
    st.write(
        "Developed a data pipeline for image segmentation using PyTorch Lightning and NVIDIA DALI. This project involved creating a custom dataset and data loader, performing data augmentation and normalization, training a DeepLabV3+ model, and visualizing the segmentation output."
    )
    st.markdown(
        "**Tech Stack**: Python, PyTorch, PyTorch Lightning, NVIDIA DALI, NumPy, Pandas, OpenCV"
    )
